...
const accessGranted = readStoredValue('wedding-access-v1') === 'true';
...
const uiTranslations = {
  ...
  conventLabel: ['Rooftop Restaurants', 'Rooftop Restaurant'], conventCaption: ['Stone courtyards and centuries of history', 'Patios de piedra y siglos de historia'], ridgeLabel: ['Tepozteco', 'Tepozteco'], ridgeCaption: ['Dramatic mountains above the town', 'Montañas imponentes sobre el pueblo'],
  ...
  giftCopy: ['If you would still like to contribute toward our future, Canadian guests may send an e-transfer to valeriaandseanharrigan@gmail.com. There is absolutely no expectation.', 'Si aun así desean contribuir a nuestro futuro, los invitados de México pueden enviar una transferencia bancaria; compartiremos los datos de la cuenta más cerca de la fecha. No hay ninguna expectativa.'],
...
const setupPaperSelect = (select) => {
  ...
  if (select.id === 'tepoztlan-location') control.classList.add('paper-select-control--open-up');
  trigger.type = 'button';
...
passwordGateForm?.addEventListener('submit', async (event) => {
  ...
  storeValue('wedding-access-v1', 'true');
